...
SkillOps Local is a local-first workbench for AI agent capabilities. It helps you scan, review, install, remove, and manage local Skills, MCP servers, and CLI tools for Codex, Claude, and project-level agent workflows.

SkillOps Local 是一个本地优先的 AI Agent 能力工作台。它把散落在本机、项目目录和常用 Agent 工具里的 Skills、MCP 服务、CLI 工具集中起来，让你能看见能力来源、健康状态、风险信息和安装路径。
...
- Scans local and project-level Skills, MCP servers, and CLI tools.
- Reviews `SKILL.md` metadata, runtime needs, permissions, and common risk patterns.
- Manages installed Skills with local remove and trash-based recovery flows.
- Installs Skills from the bundled market or GitHub sources.
- Opens a local Web UI for inventory, risk review, MCP inspection, CLI tool management, history, and recycle bin workflows.
- Keeps user data local by default; the CLI and desktop client run against local files and local configuration.

## Release Channels

- `npm` CLI package for terminal-first workflows.
- macOS M-series desktop client built with Wails and Go.
- Windows desktop client built with Wails and Go.
- GitHub Releases for versioned desktop packages and checksums.
